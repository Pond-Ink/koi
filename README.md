...
事件标准化（含引用消息） → 去重 → 同群串行队列
        ...
        │
        ├─ 全量普通群消息 ─→ OpenAI 严格判断是否回复 ─→ 静默记忆或回复/本地命令
        │
...
- `src/ai/`：构造模型上下文、严格判断是否回复和选择允许的工具，不实现命令业务。
...
- 其余 `GROUP_MESSAGE_CREATE` 消息会交给 AI 严格判断：只有内容明显面向机器人、明显希望机器人回复，或明显在与机器人互动时才回复；其余情况必须静默并写入记忆。
- AI 会收到 `@` 的结构化信息和被回复消息是否来自机器人，作为上述判断依据；提及机器人名字、泛泛谈论 AI 或“可以补充观点”都不足以主动插话。
